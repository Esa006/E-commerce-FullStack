    ...
    try {
      ...
      if (response.data.success) {
        Swal.fire({
          ...
          text: 'Thank you for shopping with FOREVER.',
          icon: 'success',
          confirmButtonColor: '#000',
      ...
      if (error.response && error.response.status === 422) {
        ...
        Swal.fire({ title: 'Input Error', text: errorMessages, icon: 'warning', confirmButtonColor: '#000' });
      } else {
        const status = error.response?.status;
        const msg = error.response?.data?.message || error.message;
        Swal.fire({ title: `Error ${status || ''}`, text: msg, icon: 'error', confirmButtonColor: '#000' });
      }
    }
  };
