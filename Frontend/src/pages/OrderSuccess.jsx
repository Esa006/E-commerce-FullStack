...
export default function OrderSuccess() {
    ...
    useEffect(() => {
        ...
        if (stateData && stateData.id && stateData.items && stateData.items.length > 0) {
            // We have order data WITH items from navigation state - use it directly
            console.log('Using order data from navigation state');
            setOrderData(stateData);
            setLoading(false);
        } else if (stateOrderId || (stateData && stateData.id)) {
            // We have order ID but incomplete data (missing items) - fetch full details
            const idToFetch = stateOrderId || stateData.id;
            console.log('Fetching full order details for ID:', idToFetch);
